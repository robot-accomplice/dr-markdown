...
func EmitJS(methods []Method, events []string) string {
	...
	b.WriteString("// `raw` mirrors the Go method names EXACTLY and carries no semantics.\n")
	b.WriteString("// Defaults, fallbacks and warn-versus-silent behaviour are application\n")
	b.WriteString("// concerns and live in bridge.js, which is hand-written on purpose.\n")
	b.WriteString("export const raw = {\n")
...
- [ ] **Step 1: Point `bridge.js` at the generated layer, changing nothing else**

`bridge.js` is a **semantic** adapter, not a name mapping, and its per-method quirks are load-bearing
and already understood: `recordEvent` maps to `RecordClientEvent` and defaults `fields ?? {}`;
`frontendReady` falls back to `Promise.resolve([])` because the boot path iterates the result;
`loadPreferences`/`savePreferences` return `null` silently; `setDirty`/`updateContent` are silent.
Those are application concerns, not host concerns, and they stay **verbatim**.

The only change is what the module talks to. Replace the `wails()` helper at the top:

```js
const wails = () => globalThis.go?.main?.App ?? null
```

with the generated, host-free surface:

```js
import { raw } from './host/bridge.generated.js'

// Was `globalThis.go?.main?.App`. The host now lives behind transport.js, and
// `raw` is generated from app.go so these names cannot drift from Go.
const wails = () => (raw.available() ? raw : null)
```

then rewrite each entry to call `raw.<Method>` instead of `wails()?.<Method>`, **keeping every
fallback, default and comment exactly as written**. For example:

```js
  recordEvent: (event, fields) => raw.RecordClientEvent?.(event, fields ?? {}),
  frontendReady: () => raw.FrontendReady?.() ?? Promise.resolve([]),
  loadPreferences: () => raw.LoadPreferences?.() ?? null,
```

This keeps `bridge.js`'s public shape and behaviour identical while removing the last direct host
reference from it.
